...
public abstract class ContainerBase extends LifecycleMBeanBase implements Container {
    ...
    public synchronized void addValve(Valve valve) {
        log.info("addValve---------解析设置对应的service和对应的valve="+ valve.getClass().getName());
    ...
    protected void threadStart() {
        if (backgroundProcessorDelay > 0 &&
            ...
            System.out.println("每10秒定时获取一次Service相关配置信息--------开始");
    ...
    protected class ContainerBackgroundProcessorMonitor implements Runnable {
        @Override
        public void run() {
            boolean available = getState().isAvailable();
            System.out.println("每60秒执行一次，available="+available);
            if (getState().isAvailable()) {
                threadStart();
            }
        }
    ...
    protected class ContainerBackgroundProcessor implements Runnable {
        ...
        @Override
        public void run() {
            System.out.println("定时任务执行----------每10秒一次");
